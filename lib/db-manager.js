var config = require('./db-config')
  , db = config.db
  , fs = require('fs')
  , x2j = require('./git/xml2json').xml2json
  //, x2j = require('nodeutil').xml2json
...
function generateReport(rpt, params, callback) {
  ...
  /* Add end sql append to contain statments like "group by", "order by" */ 
  if(conf.endsql) {
    sql += (' ' + conf.endsql);
  }
  ...
  if(!sql) {
    log.error('sql not validate...');
    callback({"STATUS":"Error", "MSG":"sql not validate..."});
  } else
  db.query(sql, cond, function(err, rows, fiels) {
    if(err) {
      log.error(err);
      callback({"STATUS":"Error", "MSG":"query db error..., error sql:" + sql + ', condition:' + condition});
    } else {
      if(conf.replacement){
      //TODO replace the result data
      }
