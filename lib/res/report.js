 ...
 */
(function($) {
$.fn.genReport2Form = function(rptRoot, rpt, query, filterRule, replaceMap) {
  var url = rptRoot + '/' + rpt + '?ts=' + new Date().getTime() + getQueryString(query);
  var _this = $(this);
  genReport2Form(_this, url, filterRule, replaceMap);
};
})(jQuery);

function genReport2Form(_this, url, filterRule, replaceMap) {
  $.ajax({
      url: url,
      success: function(data) {
        if(data && data.length > 0) {
          ...
          for(var i = 0 ; i < keys.length ; i++){
            ...
            if($('#' + k))
              ...
              $('input[name=' + k + '][value='+d[0][keys[i]]+']').attr('checked','checked');
          }
        } else {
          _this.html('<p id="rpt_msg" style="color:red">No data for present!</p>');
        }
