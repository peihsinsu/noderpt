...
function getRptFromConfig(rpt, query, cb){
  log.debug('Do report of : %s', rpt);
  log.debug('url_parts.query : %s', JSON.stringify(query));
  dbm.generateReport(rpt, query, function(err, rows, fiels){
    cb(rows);
  });
}
exports.getRptFromConfig = getRptFromConfig;
